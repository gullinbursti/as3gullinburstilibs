...
const wxUint32 CRC32::crc_table[256] =
{
 ...
 */
wxString CRC32::getValue(const bool hexInUpperCase) const
{
  wxString h;
  if (hexInUpperCase)
    h = wxT("%08X");
  ...
  return wxString::Format(h, getUint32Value());
}
//---------------------------------------------------------------------------
 ...
 */
void CRC32::update(const wxByte* buf, unsigned int len)
{
  if (buf == NULL || len == 0)
    return;
